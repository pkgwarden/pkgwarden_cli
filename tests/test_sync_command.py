...
def _write_tape_config(tmp_path: Path, manager: str = "uv") -> None:
...
def test_sync_tape_success_runs_uv_sync(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    _write_tape_config(tmp_path, "uv")
...
def test_sync_tape_failure_prints_guidance_and_exits_nonzero(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.chdir(tmp_path)
...
def test_sync_respects_configured_manager_override(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    _write_tape_config(tmp_path, "pnpm")
