...
def sync(
    ...
    """Install project dependencies; on failure, upload the lockfile for approval."""
    ...
    try:
        first = run_process(sync_command(manager), cwd=cwd, env_overrides=auth_env or None)
        _echo_process(first)
        ...
        if not fallback(runtime, manager, cwd):
            raise typer.Exit(1)
        second = run_process(sync_command(manager), cwd=cwd, env_overrides=auth_env or None)
...
def _print_gate_guidance() -> None:
    typer.echo(
        "pw sync: native resolution failed. "
        "Run `pw why-blocked <pkg>==<version>` for a pinned spec, or "
        "`pw resolution-insights <package>` for gate index version coverage.",
        err=True,
    )
